...
public class RaceSimTools {
	...
	/**
	 * Checks if a GridCell is within the map borders and does not collide with
	 * an obstacle. If isWild == true, obstacle check is skipped
	 * 
	 * @param g
	 *            The GridCell to check
	 * @param track
	 * @param isWild
	...
	/**
	 * Samples a resulting new state. Does not use function nextStates as doing
	 * so would require unnecessary computation
	 * 
	 * @param state
	 *            The current state
	 * @param actions
	 ...
	 * @return
	 */
	public static RaceState sampleNextState(RaceState state,
	 ...
	 * @param adv1Plus
	 *            If true, TO action is allowed. Should be true iff number of
	 *            cycles in the race > 1.
	...
	public static Player sampleNextPlayer(Player player, Track track,
		...
		if (action == Action.TO) {
			...
			if (r <= 0.7) { // Success
				nextCell = cell.shifted(action == Action.NE ? Direction.NE
						: Direction.SE);
			} else if (r <= 0.8) { // Fail and go N/S
				nextCell = cell.shifted(action == Action.NE ? Direction.N
						: Direction.S);
		...
		return new Player(player.getId(), player.getCycle(), nextCell,
				damageCost, willBeObstacle);
	}

	public static Player sampleNextPlayer2(Player player, Track track,
			List<Distractor> distractors, Action action, Random random,
			boolean adv1Plus) {
		action = restrictToValidActions(player, action, adv1Plus);
	...
	public static Opponent sampleNextOpponent(Opponent opponent,
		...
		Action desiredAction = chooseRandom(opponent.getPolicy().get(cell),
				random);
		GridCell nextCell = furthestMove(cell, desiredAction, track,
	 ...
	 * 
	 * @param state
	 *            The current RaceState
	 * @param actions
	 *            List of actions (one for each player)
	 * @param track
	 * @param random
	 *            A random number generator
	...
	public static Map<RaceState, Double> nextStates(RaceState state,
		...
		for (int i = 0; i < numPlayers; i++) {
			...
			for (int j = 0; j < leaves.size(); j++) {
				...
				// Add a new child to this leaf for each possible next player
				// state
		...
		for (Opponent o : state.getOpponents()) {
			...
			for (int i = 0; i < leaves.size(); i++) {
				...
				List<Actor> ancActors = parent.getDataAncestry();
				ListIterator<Actor> it = ancActors
						.listIterator(1 + numDistractors);
				for (int j = 0; j < numPlayers; j++) {
					ancPlayers.add((Player) it.next());
				}
				Map<GridCell, Double> no = nextOpponentPositions(o, ancPlayers,
						track);
				for (Map.Entry<GridCell, Double> entry : no.entrySet()) {
					Node<Actor> child = new Node<Actor>(new Opponent(o.getId(),
							o.getPolicy(), entry.getKey()));
					parent.addChild(child);
					newLeaves.add(child);
					newLeafProb.add(leafProb.get(i) * entry.getValue());
				}
	...
	/**
	 * Restricts the action to make sure it is a valid action for the current
	 * player/track.
	 */
	public static Action restrictToValidActions(Player player, Action action,
			boolean adv1Plus) {
	...
	/**
	 * Returns the probability distribution over possible player states for the
	 * next turn
	 * 
	 * @param player
	 * @param action
	...
	/**
	 * Returns the probability distribution over possible opponent cells for the
	 * next turn
	 * 
	 * @param opponent
	 * @param players
	 *            List of players, to check if any are in obstacle mode
	 * @param track
	 * @param distractors
	 * @return Map<Opponent, Double> where the Double is the probability
	 */
	public static Map<GridCell, Double> nextOpponentPositions(
			Opponent opponent, List<Player> players, Track track) {
		...
		for (Map.Entry<Action, Double> entry : actionMap.entrySet()) {
			...
			GridCell g = furthestMove(currentPos, action, track,
					extraObstacles, false);
			Opponent o = new Opponent(opponent.getId(), opponent.getPolicy(), g);
			if (cellDistribution.containsKey(g)) {
				cellDistribution.put(g,
						cellDistribution.get(g) + entry.getValue());
			} else {
	 ...
	 * @param track
	 * @param distractors
	 * @param extraObstacles
	 *            List of obstacle grid cells in addition to those listed in the
	 *            track's map. Used for player cycles in obstacle mode
	 * @param isWild
	 * @return
	 */
	 ...
	 */
	public static boolean isDistracted(GridCell pos,
	 ...
	 * 
	 * @param input
	 *            Map<K, Double>, where Double is probability of choosing K
	 * @param random
	 *            Random number generator
	 * @return randomly chosen K. May return null if probabilities do not sum to
	 *         1
	 */
	...
	public static String stateToString(RaceState state, Track track) {
		...
		HashMap<GridCell, ArrayList<String>> things = new HashMap<GridCell, ArrayList<String>>();
