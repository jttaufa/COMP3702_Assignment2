...
public class Tour {
	...
	public boolean buyCycle(Cycle cycle) {
		if (raceNo != 0 || status != Status.PREPARING) {
			System.out
					.println("Error: You can only buy cycles at the start of the tour");
			return false;
	 ...
	 * 
	 * @param track
	 *            The track to register for.
	 * @param numPlayers
	 *            The number of players to register for
	 * @return true iff success
	 */
	public boolean registerTrack(Track track, int numPlayers) {
		if (raceNo != 0 || status != Status.PREPARING) {
			System.out
					.println("Error: You can only register at the start of the tour");
			return false;
			...
			System.out
					.println("Error: Already registered for max number of races");
			return false;
		...
		if (numPlayers < 1 || numPlayers > maxPlayers) {
			System.out
					.println("Error: Invalid number of players for registration");
			return false;
		}
	 ...
	 * 
	 * @param track
	 *            The chosen track. Must be registered
	 * @param players
	 *            The players, with the cycles and start positions set
	...
	public boolean startRace(Track track, List<Player> players) {
		...
		System.out
				.println("Started race on track " + track.getFileNameNoPath());
	 ...
	 * 
	 * @param actions
	 *            List of actions. Size must match number of players.
	 */
	...
	/**
	 * Return the latest race state. Returns null if no race state exists in
	 * history.
	 * 
	 ...
	 * 
	 * @param raceIndex
	 *            index of RaceSim to get history from. Should be <
	 *            getNumRaces()
	...
	public List<RaceState> getStateHistory(int raceIndex) {
		if (raceIndex < 0 || raceIndex > raceSims.size()) {
			System.out
					.println("Error: cannot get history; race does not exist");
			return null;
		}
	 ...
	 * 
	 * @param raceIndex
	 *            index of RaceSim to get track from.
	 * @return Track
	 */
	...
	public int getTurnNo(int raceIndex) {
		if (raceSims.size() <= raceIndex) {
			System.out
					.println("Error: can't get no. of turns, race doesn't exist");
			return -1;
		} else {
	...
	public boolean loadFromFile(String filename) throws IOException {
		...
		try {
			...
			while (true) {
				...
				HashMap<Character, RandomPolicy> policies = new HashMap<Character, RandomPolicy>();
				...
				List<ArrayList<Action>> actionHistory = new ArrayList<ArrayList<Action>>();
				for (int turnNo = 0; turnNo < numStates; turnNo++) {
					...
					Map<String, GridCell> playerPos = new HashMap<String, GridCell>();
